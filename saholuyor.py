        ...
        for cell in row:
            if cell.value != None:
                if "Siyah" in cell.value:
                     ...
                     sheet = kitap.active
                     deger,x,y=tasa_baglanma_siyah(sheet.cell(cell.row, 2), sheet.cell(cell.row, 3), cell.value,sheet,0,0)
                     if deger ==61:
                          liste=beyaz_sira_degerli(sheet,kitap,b)
                          konumlar=konumlar+liste
                          kitap.close()
    konumlar=remove(konumlar)
    if konumlar!=0:
        sahmatmi=siyah_sahmat_yapiyor(sheet,kitap,konumlar)
    return konumlar,sahmatmi


def siyah_sahmat_yapiyor(sheet,kitap,liste2):
    sahmatmi=[]
    for row in sheet.iter_rows(min_row=2, max_row=33, min_col=1, max_col=1):
        for cell in row:
            if cell.value != None:
                if "Beyaz" in cell.value:
                    ...
                    for i in range(len(liste2)):
                        if liste2[i][0]==cell.value:
                            a.remove(((liste2[i][1],liste2[i][2])))
                    sahmatmi=sahmatmi+a
    return sahmatmi
...
def siyah_sira(sheet,kitap):
        ...
        for cell in row:
            if cell.value != None:
                if "Beyaz" in cell.value:
                     kitap = load_workbook("satranc.xlsx")
                     sheet = kitap.active
                     deger,x,y=tasa_baglanma_beyaz(sheet.cell(cell.row, 2), sheet.cell(cell.row, 3), cell.value,sheet,0,0)
                     if deger ==61:
    ...
    konumlar=remove(konumlar)
    if konumlar!=0:
        sahmatmi=beyaz_sahmat_yapiyor(sheet,kitap,konumlar)
    return konumlar,sahmatmi


def beyaz_sahmat_yapiyor(sheet,kitap,liste2):
    sahmatmi=[]
    for row in sheet.iter_rows(min_row=2, max_row=33, min_col=1, max_col=1):
        for cell in row:
            if cell.value != None:
